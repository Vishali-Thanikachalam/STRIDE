...
int main(void)
{
  ...
  HAL_TIM_Base_Start_IT(&htim2);       /* TIM1: 100ms IRQ      */
  HAL_TIM_Base_Start_IT(&htim3);   /* counter every 1us */
  ...
  /* Seed SOC from PA2 potentiometer at boot only */
  float seed_raw = Read_ADC_Channel(ADC_CHANNEL_2);
  float seed_pct = (seed_raw / 4095.0f) * 100.0f;
  EV_InjectSOC(&ev, seed_pct);
